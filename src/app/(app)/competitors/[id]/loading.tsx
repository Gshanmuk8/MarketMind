...
/** Terminal-styled loading for the dossier — no light flash before the dark file. */
export default function Loading() {
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-px bg-[var(--t-line)] sm:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="bg-[var(--t-bg)] px-5 py-6 sm:px-7">
            <div className="shimmer h-3 w-16 rounded bg-white/5" />
            <div className="shimmer mt-4 h-8 w-14 rounded bg-white/5" />
          </div>
        ))}
      </div>
      <div className="space-y-3 px-5 py-6 sm:px-7">
        {[0, 1, 2].map((i) => (
          <div key={i} className="shimmer h-20 rounded-xl bg-white/[0.03]" />
        ))}
      </div>
    </div>
  );
